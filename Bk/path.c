...
/**
 * _which - a function that searches for a file
 * @filename: file name
 * @head: head of linked list of directories (path)
 *
 * Return: path of file
 */

char *_which(char *filename, PathNode *head)
{
	struct stat st;
	char *str;
	...
	while(temp)
	{
		str = concat(temp->directory, "/", filename);
		if (stat(str, &st) == 0)
			return (str);
		free(str);
		temp = temp->next;
